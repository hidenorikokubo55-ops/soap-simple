...
.sc.sp2 .sh { background:#f3e5f5; border-bottom:1px solid #e1bee7; }
...
.sc.ss .sn{color:#1565c0} .sc.so .sn{color:#2e7d32} .sc.sa .sn{color:#e65100} .sc.sp2 .sn{color:#6a1b9a}
.sn2 { font-size:10px; color:var(--muted); }
.sb { padding:14px; min-height:80px; }
.scont { font-size:13px; line-height:1.9; color:var(--text); white-space:pre-wrap; font-family:inherit; }
.sk { background:linear-gradient(90deg,#eee 25%,#ddd 50%,#eee 75%); background-size:200% 100%; animation:shimmer 1.5s infinite; border-radius:4px; height:13px; margin-bottom:8px; }
@keyframes shimmer{0%{background-position:200% 0}100%{background-position:-200% 0}}
.sa2 { display:flex; gap:10px; justify-content:flex-end; margin-top:14px; }
...
<nav>
  <button class="tab active" onclick="showTab('input')">新規カルテ作成</button>
  <button class="tab" onclick="showTab('history')" id="htab">カルテ履歴 <span class="badge" id="hbadge">0</span></button>
</nav>
...
<main>
  ...
  <div class="card">
    ...
    <label class="uz">
      ...
      <input type="file" accept="audio/*" onchange="uploadAudio(event)" style="display:none">
    </label>
    ...
    <div class="ts">
      ...
      <textarea id="ta" rows="8" placeholder="録音後ここに文字起こしが表示されます。または直接入力してください。&#10;&#10;例：患者は右下6番が3日前から痛いと来院。咬合痛あり。打診痛+。X線にて根尖部透過像確認。本日開放創処置、抗生剤処方。次回根管治療予定。"></textarea>
  ...
  <div class="card" id="soapCard" style="display:none">
    ...
    <div class="card-body">
      <div class="sg">
        <div class="sc ss"><div class="sh"><span class="sl s">S</span><div><div class="sn">Subjective</div><div class="sn2">主訴・主観的情報</div></div></div><div class="sb"><div id="sc-s" class="scont"></div></div></div>
        <div class="sc so"><div class="sh"><span class="sl o">O</span><div><div class="sn">Objective</div><div class="sn2">客観的情報・所見</div></div></div><div class="sb"><div id="sc-o" class="scont"></div></div></div>
        <div class="sc sa"><div class="sh"><span class="sl a">A</span><div><div class="sn">Assessment</div><div class="sn2">評価・診断</div></div></div><div class="sb"><div id="sc-a" class="scont"></div></div></div>
        <div class="sc sp2"><div class="sh"><span class="sl p">P</span><div><div class="sn">Plan</div><div class="sn2">治療計画</div></div></div><div class="sb"><div id="sc-p" class="scont"></div></div></div>
      </div>
      <div class="sa2">
        <button class="bsm" onclick="saveHistory()">履歴に保存</button>
        <button class="bsm" onclick="printSoap()">印刷 / PDF</button>
      </div>
    </div>
  </div>
</main>
...
function buildChart(){
  ...
  [UPPER,LOWER].forEach((row,ri)=>{
    ...
    lbl.className='jl';lbl.textContent=ri===0?'上':'下';
    d.appendChild(lbl);
    row.slice(0,8).forEach(n=>d.appendChild(makeTooth(n)));
    const dv=document.createElement('span');dv.className='jd';
    d.appendChild(dv);
    row.slice(8).forEach(n=>d.appendChild(makeTooth(n)));
    area.appendChild(d);
  });
}

function makeTooth(n){
  const t=document.createElement('div');
  t.className='tooth';t.textContent=n;t.id='t'+n;
  t.onclick=()=>{
    const c=toothSt[n]||'';
    const next=TS[(TS.indexOf(c)+1)%TS.length];
    toothSt[n]=next;
    t.className='tooth'+(next?' '+next:'');
  };
  return t;
}
...
function getTeeth(){
  return Object.entries(toothSt).filter(([,v])=>v)
    .map(([k,v])=>k+'('+(v==='affected'?'患部':v==='treated'?'処置済':'欠損')+')')
    .join(', ');
}
...
// ── 録音 ────────────────────────────────────────────────
let mr=null,chunks=[],recOn=false,timerInt=null,sec=0;
...
async function sendWhisper(blob){
  ...
  try{
    ...
    toast('文字起こし失敗。テキストを直接入力してください。','err');
  }
...
async function analyze(){
  ...
  btn.disabled=true;
  btn.innerHTML='<div class="sp"></div> AIが分析中...';
  ...
  try{
    ...
    document.getElementById('errMsg').textContent='エラー: '+e.message;
  }
  btn.disabled=false;
  btn.textContent='SOAP形式でAI分析・カルテ生成';
}
...
function saveHistory(){
  ...
  document.getElementById('hbadge').textContent=history.length;
  toast('履歴に保存しました');
}

function renderHistory(){
  const el=document.getElementById('histList');
  if(!history.length){el.innerHTML='<div class="card"><div class="empty">まだ保存されたカルテがありません。</div></div>';return;}
  el.innerHTML=history.map(e=>\`
<div class="card hi">
  <div class="hh">
    <div>
      <span class="hd">\${e.date}</span>
      <span class="hn">\${e.name}\${e.dob?' / '+calcAge(e.dob):''}  \${e.gender||''}</span>
      \${e.doctor?'<span class="hm">担当: '+e.doctor+'</span>':''}
    </div>
    <div class="hb">
      <button class="bsm" onclick="printEntry(\${e.id})">印刷</button>
      <button class="bsm" onclick="delEntry(\${e.id})">削除</button>
    </div>
  </div>
  <div class="hsoap">
    \${['S','O','A','P'].map(l=>\`<div><div class="hsi-l \${l.toLowerCase()}">\${l}</div><div class="hsi-t">\${e.soap[l]||'——'}</div></div>\`).join('')}
  </div>
</div>\`).join('');
}
...
function printSoap(){
  ...
  printEntry({id:0,name,dob,gender:document.getElementById('pGender').value,date,doctor:document.getElementById('pDoc').value,insurance:document.getElementById('pIns').value,teeth:getTeeth(),soap:currentSoap});
}
